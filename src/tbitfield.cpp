...
{
    elType res = 0;
    if (getLength() >= sizeof(elType) * 8)
        res = res | 1 << (n/2 % (sizeof(elType)*8));
    else
        res = res | 1 << (n % (sizeof(elType) * 8));
    return res;
}
...
{
    if (bitLen != 0)
        return memLen * sizeof(elType) - 8;
    else return memLen * sizeof(elType);
}
